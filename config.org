...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; Run blocks like in MATLAB. Needs work.
  (defun my/python-run-block ()
    "Run the current block"
    (interactive)
    (let ((p0 (save-excursion (or (re-search-backward "^\s*#" (point-min) t) (point-min))))
          (p1 (save-excursion (or (re-search-forward "^\s*#" (point-max) t) (point-min)))))
      (python-shell-send-region p0 p1)
      (goto-char p1) (next-line))
    )

  (define-key python-mode-map (kbd "C-<return>") 'my/python-run-block)
#+end_src
** RealGUD
